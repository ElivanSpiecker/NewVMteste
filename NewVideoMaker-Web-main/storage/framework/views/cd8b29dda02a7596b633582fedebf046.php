...
<?php $__env->startSection('description', 'Configure credenciais, pipeline e veja status dos serviços.'); ?>

<?php $__env->startSection('content'); ?>
<div class="min-h-screen p-8 lg:p-12">
    <div>
        <h1 class="font-display text-4xl font-bold tracking-tight text-foreground lg:text-5xl">Configurações</h1>
        <p class="mt-2 text-sm text-muted-foreground">Tudo que o aplicativo precisa para funcionar — sem editar arquivos no disco.</p>
    </div>
    ...
    <div class="mt-10 max-w-4xl space-y-10">
        ...
        <section class="card">
            <h2 class="section-title">Estado geral</h2>
            <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <?php $__currentLoopData = [
                    ['label' => 'Serviços Python',     'ok' => $servicosOk, 'href' => '#servicos'],
                    ['label' => 'Pipeline configurado', 'ok' => $pipelineOk, 'href' => '#pipeline'],
                    ['label' => 'YouTube conectado',    'ok' => $youtubeOk,  'href' => '#youtube'],
                    ...
                    <a href="<?php echo e($item['href']); ?>" class="flex items-center gap-3 rounded-sm border border-border bg-background p-3 transition-colors hover:bg-accent">
                        ...
                        <span class="ml-auto text-[10px] uppercase tracking-wider text-muted-foreground"><?php echo e($item['ok'] ? 'OK' : 'Pendente'); ?></span>
                    </a>
        ...
        <section id="pipeline" class="card">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">Caminhos para o Python do venv, o pipeline.py e a pasta de saída.</p>
                </div>
                <span class="rounded-sm px-2 py-1 text-[10px] uppercase tracking-wider <?php echo e($pipeline_status['ok'] ? 'bg-primary text-primary-foreground' : 'bg-destructive text-destructive-foreground'); ?>">
                    <?php echo e($pipeline_status['ok'] ? 'Configurado' : 'Pendente'); ?>
            ...
            <form method="POST" action="<?php echo e(route('config.pipeline')); ?>" class="mt-5 space-y-4">
                <?php echo csrf_field(); ?>
                <div>
                    <label class="form-label" for="python_path">Python (executável do venv)</label>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    <label class="form-label" for="output_dir">Pasta de saída</label>
...
unset($__errorArgs, $__bag); ?>
        ...
        <section id="servicos" class="card">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h2 class="section-title">Serviços locais</h2>
                    <p class="mt-1 text-xs text-muted-foreground">ComfyUI, ACE-Step e Ollama precisam estar rodando para gerar vídeos.</p>
                </div>
                <a href="<?php echo e(route('health.index')); ?>" class="btn-outline">Detalhes</a>
            </div>
            ...
            <div class="mt-5 space-y-3">
                <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $svc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-start justify-between gap-3 border-b border-border pb-3 last:border-0">
                        ...
                        <?php if(!$svc['up']): ?>
                            <a href="<?php echo e($svc['install']); ?>" target="_blank" rel="noopener" class="text-[11px] text-muted-foreground underline hover:text-foreground">Como instalar ↗</a>
                        <?php endif; ?>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        ...
        <section id="youtube" class="card">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h2 class="section-title">Credenciais do YouTube (OAuth 2.0)</h2>
                    <p class="mt-1 text-xs text-muted-foreground">Obrigatório para publicar e agendar Shorts.</p>
                </div>
                <span class="rounded-sm px-2 py-1 text-[10px] uppercase tracking-wider <?php echo e($youtubeOk ? 'bg-primary text-primary-foreground' : 'bg-destructive text-destructive-foreground'); ?>">
                    <?php echo e($youtubeOk ? 'Configurado' : 'Pendente'); ?>

                </span>
            </div>

            <details class="mt-4 rounded-sm border border-border bg-background p-4 text-xs">
                <summary class="cursor-pointer font-display text-xs uppercase tracking-wider text-foreground">Como obter Client ID e Client Secret</summary>
                <ol class="mt-3 list-decimal space-y-1.5 pl-5 text-muted-foreground">
                    <li>Acesse <a href="https://console.cloud.google.com/" target="_blank" rel="noopener" class="underline hover:text-foreground">console.cloud.google.com</a> e crie (ou abra) um projeto.</li>
                    <li>Em <strong>APIs e serviços → Biblioteca</strong>, busque e habilite "<strong>YouTube Data API v3</strong>".</li>
                    <li>Vá em <strong>APIs e serviços → Tela de consentimento OAuth</strong> e configure como <em>External</em>.</li>
                    <li>Em <strong>Credenciais → Criar credenciais → ID do cliente OAuth</strong>, escolha tipo <em>Aplicativo da Web</em>.</li>
                    <li>Em <em>URIs de redirecionamento autorizados</em>, cole exatamente:
                ...
                <div>
                    <label class="form-label" for="client_secret">
                        ...
                        <?php if($youtube['client_secret_set']): ?>
                            <span class="ml-1 normal-case tracking-normal text-chart-2">· salvo</span>
                        <?php endif; ?>
                    </label>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    ...
                    <input id="redirect_uri" type="url" name="redirect_uri"
                           ...
                           class="form-control font-mono text-xs">
                    <p class="mt-1 text-[11px] text-muted-foreground">Esta URL precisa estar cadastrada no Google Cloud Console como redirect autorizada.</p>
