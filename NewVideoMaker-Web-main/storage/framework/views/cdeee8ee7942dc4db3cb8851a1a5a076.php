<?php $__env->startSection('title', 'YouTube Shorts — NEW VideoMaker'); ?>
<?php $__env->startSection('description', 'Agende e publique vídeos curtos diretamente no YouTube.'); ?>
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <?php if($accounts->isNotEmpty()): ?>
                <a href="<?php echo e(route('shorts.create')); ?>" class="btn-primary">
            ...
            <div class="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <?php $__currentLoopData = $accounts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $account): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="card flex items-start justify-between gap-3">
                        <div>
                            <p class="font-display text-sm font-semibold text-foreground"><?php echo e($account->display_name ?? 'Canal'); ?></p>
                            <p class="mt-1 text-[11px] text-muted-foreground">ID: <?php echo e($account->channel_id ?? '—'); ?></p>
                            <p class="mt-2 text-[11px] text-muted-foreground">
                                Token expira:
                                <?php echo e($account->expires_at ? $account->expires_at->format('d/m/Y H:i') : '—'); ?>

                            </p>
                        </div>
                        <form method="POST" action="<?php echo e(route('shorts.disconnect', $account)); ?>" onsubmit="return confirm('Desconectar este canal?');">
                            <?php echo csrf_field(); ?>
    ...
    <section class="mt-10">
        ...
        <?php if($uploads->isEmpty()): ?>
            <div class="mt-3 rounded-sm border border-dashed border-border p-10 text-center">
                ...
                <?php if($accounts->isNotEmpty()): ?>
                    <a href="<?php echo e(route('shorts.create')); ?>" class="btn-primary mt-6">
                    ...
                    <tbody>
                        <?php $__currentLoopData = $uploads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $upload): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <tr class="border-t border-border">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-foreground"><?php echo e($upload->title); ?></p>
                                    <?php if($upload->video): ?>
                                        <p class="mt-0.5 text-[10px] text-muted-foreground">vídeo gerado: <?php echo e($upload->video->tema); ?></p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 text-muted-foreground"><?php echo e($upload->account?->display_name ?? '—'); ?></td>
                                <td class="px-4 py-3 text-muted-foreground">
                                    <?php if($upload->scheduled_at): ?>
                                        <?php echo e($upload->scheduled_at->format('d/m/Y H:i')); ?>

                                    <?php else: ?>
                                        Imediato — <?php echo e($upload->created_at?->format('d/m H:i')); ?>

                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="rounded-sm px-2 py-0.5 font-display text-[10px] uppercase tracking-wider <?php echo e($statusClass); ?>"><?php echo e($upload->statusLabel()); ?></span>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex justify-end gap-2">
                                        <a href="<?php echo e(route('shorts.show', $upload)); ?>" class="rounded-sm border border-border px-2 py-1 text-[10px] uppercase tracking-wider transition-colors hover:bg-accent">Ver</a>
                                        <?php if($upload->youtubeUrl()): ?>
                                            <a href="<?php echo e($upload->youtubeUrl()); ?>" target="_blank" rel="noopener" class="rounded-sm border border-border px-2 py-1 text-[10px] uppercase tracking-wider transition-colors hover:bg-accent">YouTube</a>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Administrador\Downloads\NewVideoMaker-Web-integrado-tailwind-blade\NewVideoMaker-Web-main\resources\views/shorts/index.blade.php ENDPATH**/ ?>