...
@section('title', 'Configuração inicial — NEW VideoMaker')
@section('description', 'Vamos preparar tudo que o app precisa para gerar e publicar vídeos.')
        ...
        @if (($source ?? null) === 'installer-payload')
            <div class="mt-6 rounded-sm border border-border bg-card px-4 py-3 text-sm text-foreground">
                <div class="flex items-center gap-2">
                    ...
                    <span>Os componentes vieram pré-instalados no instalador e já foram configurados automaticamente. Confira abaixo e clique em <strong>Concluir</strong>.</span>
                </div>
            </div>
        @endif
        ...
        <div id="steps" class="mt-10 space-y-5">
            ...
            <section class="card" data-service="youtube">
                ...
                <div data-actions class="mt-4 flex flex-wrap items-center gap-2">
                    <a href="{{ route('config') }}#youtube" class="btn-outline">
...
@push('scripts')
<script>
const STATUS_URL  = @json(route('setup.status'));
const INSTALL_URL = @json(route('setup.install-ollama'));
const PULL_URL    = @json(route('setup.pull-model'));
const SAVE_URL    = @json(route('setup.save-path'));
const CONFIG_URL  = @json(route('config'));
